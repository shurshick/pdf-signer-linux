...
class StampProfile:
    ...
    def normalize(self):
        self.width_mm = max(40.0, min(200.0, self.width_mm))
        self.height_mm = max(15.0, min(100.0, self.height_mm))
        self.font_size = max(4.0, min(16.0, self.font_size))
        self.min_font_size = max(4.0, min(self.font_size, self.min_font_size))
        self.opacity = max(0.1, min(1.0, self.opacity))
        self.scale = max(0.1, min(2.0, self.scale))
        self.logo_scale = max(50, min(300, self.logo_scale))
